...
__device__ int sort(float* filterVector)
{
	int index = 0;

	for (int i = 0; i < sizeof(filterVector); i++) {
		for (int j = i + 1; j < sizeof(filterVector)+1 ; j++) {
		...
		if((x > 0 && x < (cols - 1)) && ((y > 0) && (y < (rows-1)))) {
			...
			for(int i = -1; i < 2; i++) {
				for(int j = -1; j < 2; j++) {
					...
					for ( int k = 0; k < 3; k ++) {
						for (int l = 0; l < 3; l++) {                   
							sum += (in[((x+i)+(y+j)*cols)*elemSize+z] - in[((x+i+k)+(y+j+l)*cols)*elemSize+z])
								* (in[((x+i)+(y+j)*cols)*elemSize+z] - in[((x+i+k)+(y+j+l)*cols)*elemSize+z]);                                    
						}
					}
					mms[ww]=sqrt(sum);
					++ww;
			...
			out[offset+z] = in[((x-1 + (ans/3)) + (y-1 + (ans%3)) * cols)*elemSize + z];
